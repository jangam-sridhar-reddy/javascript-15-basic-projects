...
function displayMenuButtons(){
  let categories = menu.reduce(function(values, menu){
    if(!values.includes(menu.category)){
      values.push(menu.category);
    }
    return values;
  }, ["all"])

  let buttons = categories.map(function(button){
    return `
  ...
  const filterButtons = buttonsContainer.querySelectorAll(".filter-btn");
    ...
    filterButtons.forEach(function(filterButton){
      filterButton.addEventListener("click", function(){
        const dataValue = this.dataset.id;
        const menuCategory = menu.filter(function(item){
          return dataValue === item.category
        })
        if(dataValue === "all"){
          displayMenuItems(menu)()
        }
